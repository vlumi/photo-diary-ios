#!/usr/bin/env bash
# Regenerate PhotoDiary.xcodeproj from project.yml.
# Refuses only if *this* project is open in Xcode: regenerating on disk while
# Xcode holds its in-memory model causes a save conflict and can clobber the
# regenerated file. Unrelated projects open in Xcode are fine.
set -euo pipefail

cd "$(dirname "$0")/.."

project_name="PhotoDiary.xcodeproj"
